...
QBC_OPS_PY="$SCRIPT_DIR/utils/qbc_ops.py"
...
if [[ ! -f "$QBC_OPS_PY" ]]; then
  echo "[ERROR] Helper script not found: $QBC_OPS_PY"
  exit 1
fi
...
for ((round=0; round<MAX_ROUNDS; round++)); do
  ...
  candidate_seed=$((GLOBAL_SEED + round))
  echo "[INFO] Round $round: selecting candidate_k=$candidate_k by random subset (seed=$candidate_seed)"

  candidate_xyz="$round_dir/candidates_random_${candidate_k}.extxyz"
  candidate_pool_indices="$round_dir/candidates_random_${candidate_k}_indices.npy"
  python "$QBC_OPS_PY" random-subset \
    --pool_xyz "$pool_xyz" \
...
echo "[INFO] QBC workflow finished"
